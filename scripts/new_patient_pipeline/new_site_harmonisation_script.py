...
def run_site_harmonization(subject_ids, site_code, demographic_file_path, output_dir=BASE_PATH, dataset_newSubject=""):
    ...
    feat = Feature()
    features_smooth = [feat.smooth_feat(feature, smoothing_val) for feature, smoothing_val in smoothing_values.items()]
    ...
    c_smooth = MeldCohort(
        hdf5_file_root="{site_code}_{group}_featurematrix_smoothed.hdf5",
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ids", "--list_ids", help="Subjects IDs in a text file", default="", required=False)
    parser.add_argument(
        "-site",
        "--site_code",
        help="Site code",
