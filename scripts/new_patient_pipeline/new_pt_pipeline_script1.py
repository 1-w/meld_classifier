...
def fastsurfer_subject(subject, fs_folder, site_code=''):
    print('called fastsurfer_subject', subject)
    # # NECESSARY ?? 
    # if site_code == '':
    #     try:
...
#                    .format(fs_folder, subject,))
        ...
        T1_file = glob.glob(os.path.join(subject_dir, 'T1', '*T1w.nii*'))
        ...
        command = format("$FASTSURFER_HOME/run_fastsurfer.sh --sd {} --sid {} --t1 {} --parallel"
                    .format(fs_folder, subject, T1_file))
    
        #call fastsurfer
        print(f"INFO : Start cortical parcellation for {subject} (up to 2h). Please wait")
        ...
        proc.wait()
        print(f"INFO : Finished cortical parcellation for {subject} !")
...
def fastsurfer_flair(subject,fs_folder):
    ...
    if os.path.isfile(os.path.join(fs_folder, subject, 'mri','FLAIR.mgz')):
        print(f'STEP 1.2: Freesurfer outputs already exists for subject {subject}. \nFreesurfer will be skipped')
#         recon_all = format("$FREESURFER_HOME/bin/recon-all -sd {} -s {} -FLAIRpial -all -no-isrunning"
#                    .format(fs_folder, subject,))
        pass
    else :
        FLAIR_file = glob.glob(os.path.join(subject_dir, 'FLAIR', '*FLAIR.nii*'))
        if len(FLAIR_file)>1:
            ...
            isflair = True
        if isflair == True:
            print("Starting FLAIRpial for subject", subject)
            command = format("recon-all -sd {} -subject {} -FLAIR {} -FLAIRpial -autorecon3".format(fs_folder,subject,FLAIR_file))
            proc = sub.Popen(command, shell=True, stdout=sub.DEVNULL)
            proc.wait()
            print("finished FLAIRpial for subject", subject)
...
def extract_features(subject,scripts_dir,fs_folder, site_code=''):
    ...
    command = format(f"bash {scripts_dir}/data_preparation/meld_pipeline.sh {fs_folder} {site_code} {tmp.name} {scripts_dir}/data_preparation/extract_features {output_dir}")
...
def smooth_features(subjects,scripts_dir):
    ...
    command = format(f"python {scripts_dir}/data_preparation/run_data_smoothing_new_subjects.py -ids {subject_ids.name} -d {BASE_PATH}")
    sub.check_call(command, shell=True, stdout=sub.DEVNULL)
    subject_ids.close()
...
#                    .format(fs_folder, subject,))
        ...
        if len(FLAIR_file)>1:
            ...
            isflair = True     
        if use_fastsurfer:
            command = format("$FASTSURFER_HOME/run_fastsurfer.sh --sd {} --sid {} --t1 {}"
                    .format(fs_folder, subject, T1_file))
            if isflair == True:
                command += format("recon-all -sd {} -subject {} -FLAIR {} -FLAIRpial -autorecon3".format(fs_folder,subject,FLAIR_file))
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-id','--id_subj',
                        ...
                        default='',
                        required=False,)
    parser.add_argument('-sl','--subject_list',
                        default='',
                        help='Relative path to subject List containing id and site_code.',
                        required=False,)
    parser.add_argument('-bd','--bids_dir',
                        default='',
    ...
    parser.add_argument('-site','--site_code',
                        ...
                        default='',
                        required=False,)
    parser.add_argument('-fs','--fastsurfer',
