...
## that contains the T1 in nifti format ".nii.gz" and where available a FLAIR
## folder that contains the FLAIR in nifti format ".nii.gz"
...
def fastsurfer_subject(subject, fs_folder, verbose=False):
    ...
    if os.path.isdir(opj(fs_folder, subject_id)):
        print(
            get_m(
                f"Freesurfer outputs already exists for subject {subject_id}. Freesurfer will be skipped",
                subject_id,
                "STEP 1",
            )
        )
    ...
    if subject_t1_path == "":
        ...
        if len(subject_t1_path) > 1:
            raise FileNotFoundError(
                get_m(
                    f"Find too much volumes for T1. Check and remove the additional volumes with same key name",
                    subject,
                    "ERROR",
                )
            )
            ...
            raise FileNotFoundError(
                get_m(f"Could not find T1 volume. Check if name follow the right nomenclature", subject, "ERROR")
            )
        else:
    ...
    command = format(
        "$FASTSURFER_HOME/run_fastsurfer.sh --sd {} --sid {} --t1 {} --parallel --batch 1 --run_viewagg_on gpu".format(
            fs_folder, subject_id, subject_t1_path
        )
    )
...
def freesurfer_subject(subject, fs_folder, verbose=False):
    ...
    if subject_t1_path == "":
        ...
        if len(subject_t1_path) > 1:
            ...
            raise FileNotFoundError(
                get_m("Could not find T1 volume. Check if name follow the right nomenclature", subject_id, "ERROR")
            )
        else:
            subject_t1_path = subject_t1_path[0]
...
def smooth_features_new_subjects(subject_ids, output_dir):
    ...
    for feature in np.sort(list(set(features))):
        ...
        smoothing.smooth_data(
            feature, features[feature], clipping_params=CLIPPING_PARAMS_FILE, outliers_file=outliers_file
        )

    tmp.close()


def run_subjects_segmentation_and_smoothing_parallel(
    subject_ids, num_procs=10, site_code="", use_fastsurfer=False, verbose=False
):
...
def run_script_segmentation(
    site_code, list_ids=None, sub_id=None, use_parallel=False, use_fastsurfer=False, verbose=False
):
    ...
    if subject_id != None:
        ...
        run_subject_segmentation_and_smoothing(
            subject_id, site_code=site_code, use_fastsurfer=use_fastsurfer, verbose=verbose
        )
    else:
        if use_parallel:
            # launch segmentation and feature extraction in parallel
            print(get_m(f"Run subjects in parallel", None, "INFO"))
            run_subjects_segmentation_and_smoothing_parallel(
                subject_ids, site_code=site_code, use_fastsurfer=use_fastsurfer, verbose=verbose
            )
        else:
            ...
            for subj in subject_ids:
                run_subject_segmentation_and_smoothing(
                    subj, site_code=site_code, use_fastsurfer=use_fastsurfer, verbose=verbose
                )
