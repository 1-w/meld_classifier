...
def load_prediction(subject_id, hdf5):
    results = {}
    with h5py.File(hdf5, "r") as f:
        for hemi in ["lh", "rh"]:
            results[hemi] = f[subject_id][hemi]["prediction"][:]
    return results
...
def load_cluster(file, subject_id):
    df = pd.read_csv(file, index_col=False)
    n_clusters = df[df["ID"] == subject_id]["n_clusters"]
...
def generate_prediction_report(
    subject_ids, experiment_folder, subjects_dir, experiment_name="ensemble_iteration", dataset=None, fold=None
):

    exp_path = os.path.join(MELD_DATA_PATH, experiment_folder)
    ...
    hemis = ["lh", "rh"]
    c = MeldCohort(hdf5_file_root=DEFAULT_HDF5_FILE_ROOT, dataset=dataset)
    subjects = [s for s in c.subjects if s.subject_id in subject_ids]
    ...
    if fold == None:
        hdf_predictions = os.path.join(exp_path, "results", f"predictions_{experiment_name}.hdf5")
        csv_results = os.path.join(exp_path, "results", f"test_results_.csv")
    else:
        hdf_predictions = os.path.join(exp_path, f"fold_{fold}", "results", f"predictions_{experiment_name}.hdf5")
        csv_results = os.path.join(exp_path, f"fold_{fold}", "results", f"test_results_.csv")
    ...
    for subject in subjects:
        ...
        subject_dir = os.path.join(subjects_dir, "input", subject.subject_id)
        ...
        for i, hemi in enumerate(["lh", "rh"]):
            ...
            for cluster in list_clust[hemi]:
                ...
                ax2.axis("off")
                #                 fig2.tight_layout()
                fig2.savefig(f"{output_dir}/saliency_{subject.subject_id}_{hemi}_c{int(cluster)}.png")
                ...
                #                 fig3.tight_layout()
                fig3.savefig(f"{output_dir}/mri_{subject.subject_id}_{hemi}_c{int(cluster)}.png")
        ...
        fig.savefig(f"{output_dir}/inflatbrain_{subject.subject_id}.png")
        ...
        # add image
        pdf.subtitle_inflat()
        im_inflat = os.path.join(output_dir, f"inflatbrain_{subject.subject_id}.png")
        ...
        for cluster in clusters:
            ...
            # add image
            im_mri = glob.glob(os.path.join(output_dir, f"mri_{subject.subject_id}_*_c{cluster}.png"))[0]
            pdf.imagey(im_mri, 50)
            # add image
            im_sal = glob.glob(os.path.join(output_dir, f"saliency_{subject.subject_id}_*_c{cluster}.png"))[0]
            ...
            pdf.custom_footer(footer_txt)
        # save pdf
        file_path = os.path.join(output_dir, f"MELD_report_{subject.subject_id}.pdf")
...
if __name__ == "__main__":
    ...
    subj_ids = np.loadtxt(args.list_ids, dtype="str", ndmin=1)

    generate_prediction_report(
        subj_ids,
        args.experiment_folder,
        subjects_dir=args.subjects_dir,
        experiment_name=args.experimant_name,
        fold=args.fold,
    )
