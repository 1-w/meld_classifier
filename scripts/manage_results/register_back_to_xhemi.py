...
def get_adj_mat(surf):
    ...
    all_edges = np.vstack([surf["faces"][:, :2], surf["faces"][:, 1:3], surf["faces"][:, [2, 0]]])
...
def register_subject_to_xhemi(subject_ids, subjects_dir, output_dir, template="fsaverage_sym", verbose=False):
    ...
    if not os.path.isdir(opj(subjects_dir, template)):
        shutil.copytree(
            opj(os.environ["FREESURFER_HOME"], "subjects", template), opj(subjects_dir, os.path.basename(template))
        )
